  ...
  return (
    <article className={`rounded-[8px] border p-4 ${toneClass}`}>
      ...
      <div className="mt-4 flex items-end justify-between gap-3">
        <p className="text-xs leading-5 text-slate-200/85">{detail}</p>
  ...
  return (
    <div className="relative min-h-[23rem] overflow-hidden rounded-[8px] border border-slate-700 bg-[#07111f]/76 p-5 shadow-[0_24px_90px_rgba(14,165,233,0.10)] backdrop-blur">
      ...
      <div className="absolute left-1/2 top-1/2 grid size-28 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-[8px] border border-sky-200/25 bg-sky-300/10 text-center shadow-[0_18px_60px_rgba(14,165,233,0.18)]">
        ...
        <span className="mt-1 block text-xs font-bold uppercase text-sky-100">Owner Today</span>
      </div>
      ...
      {radarNodes.map((node) => (
        ...
        >
          ...
          <span className="mt-1 block text-xs font-bold uppercase text-slate-400">{node.label}</span>
        </div>
      ))}

      <div className="absolute bottom-5 left-5 right-5 rounded-[8px] border border-slate-700 bg-white/[0.045] p-3">
        <p className="text-xs font-bold uppercase text-slate-400">Current focus</p>
        <p className="mt-2 text-sm font-semibold leading-6 text-white">{data.brief.focus}</p>
      </div>
    </div>
  );
}
      ...
      <div className="grid min-h-[33rem] gap-0 xl:grid-cols-[minmax(0,1fr)_27rem] 2xl:grid-cols-[minmax(0,1fr)_32rem]">
        <div className="relative overflow-hidden p-6 md:p-8">
          ...
          <div className="relative mt-10 max-w-4xl">
            <p className="eyebrow">Today - {data.dateLabel} - Asia/Shanghai</p>
            ...
            <p className="mt-5 max-w-3xl text-base leading-7 text-slate-300 md:text-lg">
              The private command surface for priorities, approvals, research posture, schedule pressure, and system
        ...
        <aside className="relative border-t border-slate-700/70 bg-[#07111f]/45 p-5 md:p-6 xl:border-l xl:border-t-0">
          <TodayRadar data={data} pendingApprovals={pendingApprovals} highPressureWindows={highPressureWindows} />
          <dl className="mt-4 grid gap-3 rounded-[8px] border border-slate-700 bg-white/[0.045] p-4 text-sm">
            <div className="flex items-start justify-between gap-4">
              <dt className="text-xs font-bold uppercase text-slate-400">Private status</dt>
              <dd className="max-w-[14rem] text-right font-semibold text-white">{data.brief.posture}</dd>
            </div>
            <div className="flex items-start justify-between gap-4 border-t border-slate-700 pt-3">
              <dt className="text-xs font-bold uppercase text-slate-400">Next review</dt>
              <dd className="text-right font-semibold text-white">{data.brief.nextReview}</dd>
            </div>
            <div className="flex items-start justify-between gap-4 border-t border-slate-700 pt-3">
              <dt className="text-xs font-bold uppercase text-slate-400">Freshness</dt>
              <dd className="text-right font-semibold text-white">{data.brief.freshness}</dd>
            </div>
          </dl>
        </aside>
      </div>
...
function PriorityLane({ priority, index }: { priority: TodayPriority; index: number }) {
  return (
    <article className="grid gap-4 rounded-[8px] border border-slate-700 bg-[#07111f]/74 p-4 shadow-[0_18px_60px_rgba(2,6,23,0.18)] sm:grid-cols-[3.25rem_minmax(0,1fr)_auto] sm:items-start">
      ...
      <div className="min-w-0">
        <div className="flex flex-wrap items-center gap-2">
          <p className="text-xs font-bold uppercase text-yellow-100">{priority.lane}</p>
          <StatusBadge tone={priority.tone}>{priority.state}</StatusBadge>
        </div>
        <h3 className="mt-2 text-base font-semibold text-white">{priority.title}</h3>
        <p className="mt-2 text-xs font-bold uppercase text-sky-100">{priority.owner}</p>
...
function PriorityPanel({ priorities }: { priorities: readonly TodayPriority[] }) {
  return (
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <h2 id="today-priorities-title" className="mt-2 text-2xl font-semibold text-white">
            What Doraemon is keeping in view.
          </h2>
        </div>
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <h2 id="today-review-title" className="mt-2 text-2xl font-semibold text-white">
            {reviewQueue.length} checkpoints
          </h2>
        </div>
...
function SystemHealthRail({ systemHealth }: { systemHealth: readonly SystemHealth[] }) {
  return (
...
function SurfaceLink({ surface, index }: { surface: OperatingSurface; index: number }) {
    ...
    <>
      <div className="flex items-start justify-between gap-3">
        <div className="flex min-w-0 gap-3">
          ...
          <div className="min-w-0">
            <p className="text-xs font-bold uppercase text-slate-400">{surface.current ? "Current" : "Surface"}</p>
            <h3 className="mt-1 text-base font-semibold text-white">{surface.title}</h3>
          </div>
        </div>
        <StatusBadge tone={surface.tone}>{surface.state}</StatusBadge>
      </div>
...
function OperatingMapPanel({ operatingMap }: { operatingMap: readonly OperatingSurface[] }) {
  return (
    <section className="grid gap-5 xl:grid-cols-[22rem_minmax(0,1fr)]" aria-labelledby="today-operating-map-title">
      <div className="panel p-5 md:p-6">
        ...
        <h2 id="today-operating-map-title" className="mt-2 text-2xl font-semibold text-white">
          Public, private, research, review.
        </h2>
      ...
      <div className="grid gap-3 rounded-[8px] border border-slate-700 bg-[#07111f]/64 p-4 md:grid-cols-2 md:p-5">
        {operatingMap.map((surface, index) => (
          <SurfaceLink key={surface.title} surface={surface} index={index} />
        ))}
      </div>
    </section>
  );
    ...
    <section className="grid gap-5 lg:grid-cols-[minmax(0,0.95fr)_minmax(0,0.85fr)_minmax(0,0.8fr)]">
          ...
          {marketAlerts.map((alert) => (
            <div key={alert.label} className="rounded-[8px] border border-slate-700 bg-white/[0.045] p-3">
              <div className="flex flex-wrap items-center justify-between gap-2">
                ...
                <span className="text-xs font-bold uppercase text-sky-100">{alert.value}</span>
              </div>
      ...
      <article className="panel p-5" aria-labelledby="today-next-surfaces-title">
        ...
        <h2 id="today-next-surfaces-title" className="mt-2 text-xl font-semibold text-white">
          Move without losing the boundary.
        </h2>
        <div className="mt-4 grid gap-3">
          {commandShortcuts.map((shortcut) => (
            ...
            >
              ...
              <p className="mt-2 text-xs leading-5 text-slate-400">{shortcut.summary}</p>
...
export function OwnerTodaySurface({ data }: { data: OwnerTodaySurfaceData }) {
  ...
  return (
    <div className="grid gap-5">
      <TodayHero data={data} pendingApprovals={pendingApprovals} highPressureWindows={highPressureWindows} />
      <section className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(20rem,0.42fr)]">
        ...
        <aside className="grid content-start gap-5">
          <ReviewQueueRail reviewQueue={data.reviewQueue} />
          <SystemHealthRail systemHealth={data.systemHealth} />
        </aside>
      </section>
      <OperatingMapPanel operatingMap={data.operatingMap} />
